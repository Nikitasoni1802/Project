...
pie(diagnosis.prop.table,lables = pietables,clockwise = TRUE,col = c("purple","red"),
    radius= 0.8,cex = 1.3,main = "frequency of cancer diagnosis",border = "black")

legend(1.1,0.2, legend = diagnosis.prop.df[,2],cex = 0.7, 
       fill = c("purple","red"),border = "black")
...
ggplot(cancer, aes(x=diagnosis, y=radius_mean,fill="pink"))+
  geom_boxplot(fill="yellow")+ggtitle( "radius_mean ofbegnin")

ggplot(cancer, aes(x=diagnosis, y=area_mean,fill="pink"))=geom_boxplot()+
  ggtitle("area of begnin vs malignant")
#same for concavity(in place of area_mean)

#barplot
ggplot(cancer, aes(x=diagnosis, fill = texture_mean, fill = "yellow"))+
  geom_bar()+ggtitle("women affected in begnin and malignant")
...
ggplot(cancer, aes(x=texture_mean,fill=as.factor(diagnosis)))+
  geom_density()+ggtitle("texture mean")

#barplot for area_se 15
ggplot(cancer, aes(x=area_se>15, fill = diagnosis))+ 
  geom_bar(position = "fill")+ggtitle("area for begnin vs malignant tumour")
...
ggplot(cancer, aes(x= texture_se)) + facet_wrap(~ diagnosis)+
  ggtitle("texture se for beginin vs malignant")

ggplot(cancer, aes(x = perimeter_mean))+
  geom_histogram(fill="yellow",col = "red",binwidth = 15)+
  facet_wrap(~ diagnosis)+ggtitle("perimeter mean for begnin and malignant")
